...
class FACodecDecoder(nn.Module):
    ...
    def quantize(self, x, n_quantizers=None):
        ...
        out1, out2, out3 = None, None, None  # 初始化三个out

        # prosody (韵律量化)
        f0_input = x  # (B, d, T)
        f0_quantizer = self.quantizer[0]
        out1, q, commit, quantized = f0_quantizer(f0_input, n_quantizers=n_quantizers)
        outs += out1  # 累加到outs
        qs.append(q)
        ...
        out2, q, commit, quantized = phone_quantizer(phone_input, n_quantizers=n_quantizers)
        outs += out2  # 累加到outs
        qs.append(q)
        quantized_buf.append(quantized.sum(0))
        commit_loss.append(commit)
        ...
        if self.vq_num_q_r > 0:
            ...
            out3, q, commit, quantized = residual_quantizer(residual_input, n_quantizers=n_quantizers)
            outs += out3  # 累加到outs
            qs.append(q)
        ...
        # # 返回累加后的outs，以及三个独立的out1, out2, out3
        # return outs, out1, out2, out3, qs, commit_loss, quantized_buf
        # 返回累加后的outs，以及三个独立的out1, out2, out3
    ...
    def forward(
        ...
        if vq is True:
            ...
            x_timbre = x
            # # 调用修改后的 quantize 函数，获取 outs, out1, out2, out3 以及其他返回值
            # outs, out1, out2, out3, qs, commit_loss, quantized_buf = self.quantize(
            ...
            # 返回 outs、out1、out2、out3、vq_id（即 qs）、commit_loss、quantized 和 spk_embs
...
class FACodecDecoderV2(nn.Module):
    ...
    def forward(
        ...
        if vq is True:
            ...
            )   # quantize  的 return :outs, qs, commit_loss, quantized_buf
