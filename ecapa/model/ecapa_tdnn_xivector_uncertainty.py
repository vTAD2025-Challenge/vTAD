...
class ECAPA_TDNN_XiVec_Uncer(TopVirtualNnet):
    def init(self, inputs_dim, num_targets, isKLD=False, aug_dropout=0., tail_dropout=0., training=True,
        ...
        if pooling == "attentive":
            ...
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-kld":
            self.stats = xivec_stdinit_softplus2_prec_uncertainty_pooling_kld(pooling_params["num_nodes"], hidden_size=pooling_params["hidden_size"], stddev=False)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.manual_BN = True
            self.fc1 = ReluBatchNormTdnnLayer(
    ...
    def get_loss(self, inputs, targets):
        ...
        if len(inputs)==2: KLD on distribution
        else: only CE loss will be computed
        """
        if self.wrapper_loss is not None:
            ...
            if len(inputs) == 3:
                ...
                postmean = inputs[1]
                postcov = inputs[2]
                log_var = torch.log(postcov)

                kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - postmean ** 2 - postcov, dim = 1), dim = 0)
                ce_loss = self.loss(x, targets)
                if torch.isnan(ce_loss) or torch.isnan(kld_loss):
                    print("NaN in loss computation")
                    print(log_var)
                    print(postcov)
                loss = 0.5 * (kld_loss + ce_loss)
                return loss
            else:
