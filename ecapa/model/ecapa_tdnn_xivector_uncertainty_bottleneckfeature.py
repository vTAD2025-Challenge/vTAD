...
class ECAPA_TDNN_XiVec_Uncer_BottlenekFeature(TopVirtualNnet):
    def init(self, inputs_dim, num_targets, aug_dropout=0., tail_dropout=0., training=True,
             ...
             ecapa_params={}, dim_reduce_params={}, fc1=False, fc1_params={}, fc2_params={},
        ...
        if pooling == "attentive":
            ...
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-stdprec":
            self.stats = xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-dev":
            self.stats = xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-stdprec-vae":
            self.stats = xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_vae(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            # self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.bn_stats = None
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-stdprec-kld":
            self.stats = xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_kld(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.bn_stats = None
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-logstdprec-kld":
            self.stats = xivec_stdinit_softplus2_logprec_uncertainty_pooling_bottleneck_feature_kld(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postmean-softplus2-uncer-bottleneck-feature-stddev-kld":
            self.stats = xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature_kld(pooling_params["num_nodes"], embd_dim, hidden_size=pooling_params["hidden_size"], stddev=False, dim_reduce_params=dim_reduce_params)
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["bn_params"])
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
    ...
    def forward(self, x, x_len: torch.Tensor=torch.empty(0)):
        ...
        y = self.stats(x)
        cov = None
        frame_post_mean = None
        frame_post_cov = None
        if len(y) >= 2:
            x = y[0]
            cov = y[1]
            if len(cov.shape) != 3:
                cov = cov.unsqueeze(dim=2)
            if len(y) == 4:
                frame_post_mean = y[2]
                frame_post_cov = y[3]
        else:
        ...
        # frame_post_cov_tmp = frame_post_cov.transpose(1, 2).reshape(-1, frame_post_cov.shape[1])
        # log_var = torch.log(frame_post_cov_tmp)

        # frame_post_mean_tmp = frame_post_mean.transpose(1, 2).reshape(-1, frame_post_cov_tmp.shape[1])
        # kld_loss_utt = torch.sum(1 + log_var - frame_post_mean_tmp ** 2 - frame_post_cov_tmp, dim = 1)

        # index = torch.where(kld_loss_utt < -1000)
        # x_ind = int(index/frame_post_cov.shape[2])
        # y_ind = index - x_ind * frame_post_cov.shape[2]
        ...
        if cov is not None:
            if self.bn_stats is not None:
                cov = cov / (batch_var + self.bn_stats.eps) * torch.square(self.bn_stats.weight.view(1, self.bn_stats.weight.shape[0], 1))
            cov = torch.diag_embed(cov.squeeze(dim=2), dim1 = 1)
        ...
        if frame_post_mean == None:
            return [x]
        else:
            return x, frame_post_mean, frame_post_cov

    @utils.for_device_free
    def get_loss(self, inputs, targets, frame_post_mean=None, frame_post_cov=None):
        ...
        if self.wrapper_loss is not None:
            ...
            if not frame_post_mean == None:
                # print(frame_post_cov.shape)
                # print(frame_post_mean.shape)
                # print("size of posterior matrix: {0} {1} {2}\n".format(frame_post_cov.shape[0], frame_post_cov.shape[1], frame_post_cov.shape[2]))
                frame_post_cov = frame_post_cov.transpose(1, 2).reshape(-1, frame_post_cov.shape[1])
                log_var = torch.log(frame_post_cov)

                # print("log_var:")
                # print(log_var[0][0:10])
                # print("frame_post_mean:")
                # print(frame_post_mean[0][0:10])

                frame_post_mean = frame_post_mean.transpose(1, 2).reshape(-1, frame_post_mean.shape[1])
                kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - frame_post_mean ** 2 - frame_post_cov, dim = 1), dim = 0)
                ce_loss = self.loss(inputs, targets)
                if torch.isnan(ce_loss) or torch.isnan(kld_loss):
                    print("NaN in loss computation")
                    print(log_var)
                    print(frame_post_cov)
                loss = kld_loss + ce_loss
                # if loss > 300:
                #     print(ce_loss)
                #     print(kld_loss)
                # print("kld_loss: {0}; ce_loss: {1}".format(kld_loss.cpu(), ce_loss.cpu()))
                return loss
            else:
