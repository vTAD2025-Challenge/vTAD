...
class ECAPA_TDNN_XiVec_Uncer_Prior(TopVirtualNnet):
    ...
    def forward(self, x, x_len: torch.Tensor=torch.empty(0)):
        ...
        # standard implementation of batch norm
        # running_mean = (1 - self.bn_stats.momentum) * self.bn_stats.running_mean.view(1, self.bn_stats.running_mean.shape[0], 1) + self.bn_stats.momentum * torch.mean(x,dim=0)
        # running_variance = (1 - self.bn_stats.momentum) * self.bn_stats.running_var.view(1, self.bn_stats.running_var.shape[0], 1) + self.bn_stats.momentum * torch.var(x,dim=0)
        # output1 = (x-running_mean) / torch.sqrt(running_variance + self.bn_stats.eps) * self.bn_stats.weight.view(1, self.bn_stats.weight.shape[0], 1) + self.bn_stats.bias.view(1, self.bn_stats.bias.shape[0], 1)

        batch_mean = torch.mean(x, dim=0)
        batch_var = torch.var(x, dim=0)
        if self.bn_stats is not None:
            # x = self.bn_stats(x)
            # manually implement batch norm with pytorch params
            x = (x-batch_mean) / torch.sqrt(batch_var + self.bn_stats.eps) * self.bn_stats.weight.view(1, self.bn_stats.weight.shape[0], 1) + self.bn_stats.bias.view(1, self.bn_stats.bias.shape[0], 1)
