...
class ECAPA_TDNN_XiVec_Uncer_BottlenekFeature_Prior_Utt(TopVirtualNnet):
    def init(self, inputs_dim, num_targets, is_kld_regularizer=True, aug_dropout=0., tail_dropout=0., training=True,
        ...
        default_ecapa_params = {
            ...
            "pool_bn_params": {"momentum": 0.5, "affine": True, "track_running_stats": True}
        }
        ...
        self.mean_bn = nn.BatchNorm1d(embd_dim, **ecapa_params["bn_params"])
        self.softplus3 = torch.nn.Softplus(beta=1, threshold=20)
        self.speaker_prior_mean = torch.nn.Parameter(torch.randn(1, 128, 1), requires_grad=False)
        self.speaker_prior_cov = torch.nn.Parameter(torch.randn(1, 128, 1), requires_grad=False)
        # self.speaker_prior_mean = torch.nn.Parameter(torch.randn(1, embd_dim, 1), requires_grad=True)
        # self.speaker_prior_cov = torch.nn.Parameter(torch.rand(1, embd_dim, 1), requires_grad=True)
        ...
        self.priorcov1_relu_bn = ReluBatchNormTdnnLayer(embd_dim, embd_dim, [0], **prior_ecapa_params1)
        ...
        self.is_kld_regularizer = is_kld_regularizer
        ...
        if pooling == "attentive":
            ...
            stats_dim = self.stats.get_output_dim()
            self.bn_stats = nn.BatchNorm1d(stats_dim, **ecapa_params["pool_bn_params"])
            self.manual_BN = True
            self.fc1 = ReluBatchNormTdnnLayer(
                stats_dim, embd_dim, **fc1_params) if fc1 else None
    ...
    def get_prior_kld_loss(self, postmean, postcov):
        ...
        prior_cov = self.priorcov1_relu_bn(prior_mean)
        ...
        kld_loss_post = torch.mean(kld_loss)

        if self.is_kld_regularizer:
            kld_loss_prior = torch.mean(-0.5 * torch.sum(1 + torch.log(prior_cov) - prior_mean ** 2 - prior_cov, dim = 1), dim = 0)
        
            kld_loss = 0.5* (kld_loss_post + kld_loss_prior)
        else:
            kld_loss = kld_loss_post
