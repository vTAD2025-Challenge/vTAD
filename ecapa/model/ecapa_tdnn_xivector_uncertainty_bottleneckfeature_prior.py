...
class ECAPA_TDNN_XiVec_Uncer_BottlenekFeature_Prior(TopVirtualNnet):
    ...
    def forward(self, x, x_len: torch.Tensor=torch.empty(0)):
        ...
        y = self.stats.extract_frame_stats(x)

        post_mean_tensor = torch.transpose(y[0], 1, 2)
        post_cov_tensor = torch.transpose(y[1], 1, 2)
        post_mean = post_mean_tensor[0]
        post_cov = post_cov_tensor[0]
        for i in range(1, y[0].shape[0]):
            post_mean = torch.cat((post_mean, post_mean_tensor[i]), 0)
            post_cov = torch.cat((post_cov, post_cov_tensor[i]), 0)
    ...
    def get_prior_kld_loss(self, postmean, postcov):
        ...
        # prior_cov = self.priorcov1_1_relu_bn(prior_cov)
