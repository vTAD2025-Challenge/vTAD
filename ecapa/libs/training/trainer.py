        ...
        try:
            ...
            for this_epoch in range(start_epoch, epochs):
                ...
                for this_iter, batch in enumerate(data.train_loader, 0):
                    ...
                    gpu_id = utils.parse_gpu_id_option(self.params["gpu_id"])
                    # print(batch[0])
                    ...
                    if utils.is_main_training() and this_iter%1000==0: 
                        print("epoch: {0} iteration: {1}".format(this_epoch, this_iter))
                        self.save_model(from_epoch=False)
