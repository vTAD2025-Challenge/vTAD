    ...
    def train_one_batch(self, batch, step_lr=True):
        ...
        with context():
            ...
            with torch.cuda.amp.autocast(self.scaler is not None):
                output = model_forward(*input_list)
                if len(output) == 3:
                    loss  = model.get_loss(output[0], targets, output[1], output[2])/self.params["accum_grad"]
                else:
                    loss = model.get_loss(output[0], targets)/self.params["accum_grad"]
    ...
    def compute_validation(self, data_loader):
        ...
        with torch.no_grad():
            for idx,this_data in enumerate(data_loader):
                ...
                if len(output) == 3:
                    loss_batch = model.get_loss(output[0], targets, output[1], output[2])
                else:
                    loss_batch = model.get_loss(output[0], targets)

                loss += loss_batch.item() * len(targets)
            ...
            with model_context:
                ...
                for this_epoch in range(start_epoch, epochs+5):
                    ...
                    for _, batch in enumerate(data.train_loader, 0):
                        ...
                        # if (self.training_point[2] % 10 == 0) and utils.is_main_training():
                        #     self.save_model(mod="epoch", train_lr=self.train_lr)
