...
# For we can't get the batch_num from IterableDataset, so modified lr_scheduler                           
#                       Leo  2022-01-31)
...
class LRSchedulerWrapper():
    ...
    def __init__(self, optimizer, params:dict={}):
        ...
        if self.name == "cyclic":
            ...
            step_down = float(split_params["cyclic"]["step_size_down"]) if split_params["cyclic"]["step_size_down"] is not None else step_up
            ...
            lr_decay_step = split_params["StepLR"].pop("lr_decay_step")
            gamma = split_params["StepLR"].pop("gamma")
            self.lr_scheduler = optim.lr_scheduler.StepLR(base_optimizer, lr_decay_step, gamma)
