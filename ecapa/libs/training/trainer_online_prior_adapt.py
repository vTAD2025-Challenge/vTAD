...
class SimpleTrainer(_BaseTrainer):
    ...
    def train_one_batch(self, batch, step_lr=True):
        ...
        # print("gpu_id in SimpleTrainer: " + self.params["gpu_id"])
        ...
        with context():
            ...
            with torch.cuda.amp.autocast(self.scaler is not None):
                ...
                #     input_list = self.prev_x
                
                model_output = model_forward(*input_list)
                
                # self.prev_x = input_list

                if len(model_output) > 1:
                    postmean = model_output[0].detach()
                    postcov = model_output[1].detach()
                else:
                    output = model_output
                # loss = model.get_loss(output, targets)/self.params["accum_grad"]
                # loss = model_forward(inputs, targets)/self.params["accum_grad"]
                loss = model.get_prior_kld_loss(postmean, postcov)
        ...
        if self.num_batch%self.params["accum_grad"]==0:
            ...
            if self.params["use_amp"]:
                ...
                if not self.modify_grad():
                    ...
                    # if self.training_point[0] % 10 == 0:
                    #     print(model.layer1.affine.state_dict())
                        # print(model.speaker_prior_mean)
                        # print(model.speaker_prior_cov)
                    # if self.training_point[2] < 2:
                    #     for k, v in model.named_parameters():
                    #         print(k)
                    #         print(v)
            ...
            if self.training_point[2] == 79:
                print(self.training_point[2])
    ...
    def KLD_accumlate(self, batch, step_lr=True):
    ...
    def compute_validation(self, data_loader):
        ...
        with torch.no_grad():
            for idx,this_data in enumerate(data_loader):
                ...
                # Leo 2022-02-03
                model_output = model(*input_list)
                if len(model_output) > 1:
                    output = model_output[0]
                    postcov = model_output[1]
                else:
                    output = model_output
    ...
    def run(self):
        ...
        try:
            ...
            print("gpu_id in SimpleTrainer run: " + self.params["gpu_id"])
            if utils.is_main_training():
            ...
            # freeze_bn_params(model_forward)
            ...
            with model_context:
                ...
                for this_epoch in range(start_epoch, epochs+5):
                    ...
                    for _, batch in enumerate(data.train_loader, 0):
                        ...
                        if (self.training_point[2] % 1000 == 0) and utils.is_main_training():
                            self.save_model(mod="iter", train_lr=self.train_lr)
        ...
        try:
            ...
            with model_context:
                ...
                for _, batch in enumerate(data.train_loader, 0):
                    ...
                    postmean, postcov = self.KLD_accumlate(batch)
                    
                    if postmean_acc == None:
                        postmean_acc = postmean
                        postcov_acc = postcov
                    else:
                        postmean_acc = torch.cat((postmean_acc, postmean), 0)
                        postcov_acc = torch.cat((postcov_acc, postcov), 0)
            return postmean_acc.cpu(), postcov_acc.cpu()
...
def freeze_bn_params(model):
    for i, k in model.named_children():
        if isinstance(k, torch.nn.BatchNorm1d):
            k.eval()
        else:
            freeze_bn_params(k)