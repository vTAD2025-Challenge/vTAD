...
## Xi-vector pooling (softplus_prec)
class xivec_stdinit_softplus2_prec_uncertainty_pooling(torch.nn.Module):

    def __init__(self, input_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True):
        super(xivec_stdinit_softplus2_prec_uncertainty_pooling, self).__init__()
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        logprec = self.softplus2(self.lin2(self.lin1_relu_bn(feat)))   # frame precision estimate (standard deviation)
        prior_prec=  torch.exp(self.prior_logprec)  # log prior covariance to prior covariance 
        Ls_inverse = torch.sum(torch.cat((logprec**2, prior_prec.repeat(logprec.shape[0], 1).unsqueeze(dim=2)), 2), dim = 2) # Ls_inverse = sum(Lt) + Lp
        Ls_inverse = 1.0 / Ls_inverse
        logprec = 2.0*torch.log(logprec)                               # Square and take log before softmax                           
        ...
        if self.stddev:
            sigma2 = torch.sum(torch.cat((feat, self.prior_mean.repeat(feat.shape[0], 1).unsqueeze(dim=2)), 2).pow(2) * weight_attn, dim=2)
            sigma = torch.sqrt(torch.clamp(sigma2 - phi ** 2, min=1.0e-10)) 
            return torch.cat((phi, sigma), dim=1).unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2)
        else: 
            return phi.unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2)
...
## Xi-vector pooling (softplus_prec)
class xivec_stdinit_softplus2_prec_uncertainty_pooling_kld(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        prec = self.softplus2(self.lin2(self.lin1_relu_bn(feat)))   # frame precision estimate (standard deviation)
        prior_prec=  torch.exp(self.prior_logprec)  # log prior covariance to prior covariance 
        Ls_inverse = torch.sum(torch.cat((prec**2, prior_prec.repeat(prec.shape[0], 1).unsqueeze(dim=2)), 2), dim = 2) # Ls_inverse = sum(Lt) + Lp
        Ls_inverse = 1.0 / Ls_inverse
        logprec = 2.0*torch.log(prec)                               # Square and take log before softmax                           
        ...
        if self.stddev:
            sigma2 = torch.sum(torch.cat((feat, self.prior_mean.repeat(feat.shape[0], 1).unsqueeze(dim=2)), 2).pow(2) * weight_attn, dim=2)
            sigma = torch.sqrt(torch.clamp(sigma2 - phi ** 2, min=1.0e-10)) 
            return torch.cat((phi, sigma), dim=1).unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, (1/prec**2)
        else: 
            return phi.unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, (1/prec**2)

    def get_output_dim(self):
        return self.output_dim


## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature, self).__init__()
 
        
        default_dim_reduce_params = {
        ...
        dim_reduce_params = utils.assign_params_dict(
            default_dim_reduce_params, dim_reduce_params)
        ...
        self.prior_mean = torch.nn.Parameter(torch.zeros(1, output_dim), requires_grad=train_mean)
        self.prior_logprec = torch.nn.Parameter(torch.zeros(1, output_dim), requires_grad=train_prec)
        self.softmax = torch.nn.Softmax(dim=2)
   
        # dimension reduction
        self.dim_reduce = ReluBatchNormTdnnLayer(input_dim, output_dim, context, **dim_reduce_params)
        # Log-precision estimator
        self.lin1_relu_bn = ReluBatchNormTdnnLayer(output_dim, hidden_size, context)
        self.lin2 = TdnnAffine(hidden_size, output_dim, context=context)
        self.softplus2 = torch.nn.Softplus(beta=1, threshold=20)
    ...
    def extract_frame_stats(self, inputs):
        """
        @inputs: a 3-dimensional tensor (a batch), including [samples-index, frames-dim-index, frames-index]
        """
        assert len(inputs.shape) == 3
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature, self).__init__()
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        dev = self.softplus2(self.lin2(self.lin1_relu_bn(feat)))   # frame covariance estimate
        prior_prec=  torch.exp(self.prior_logprec)  # log prior covariance to prior covariance 
        Ls_inverse = torch.sum(torch.cat((1/(dev), prior_prec.repeat(dev.shape[0], 1).unsqueeze(dim=2)), 2), dim = 2) # Ls_inverse = sum(Lt) + Lp
        Ls_inverse = 1.0 / Ls_inverse
        logprec =  (-1.0) * torch.log(dev)                               # Square and take log before softmax                           
    ...
    def extract_frame_stats(self, inputs):
        ...
        # Log-precision estimator
        cov = self.softplus2(self.lin2(self.lin1_relu_bn(feat)))   # frame precision estimate (standard deviation)
        # cov = 1 / prec
        # cov = 1 / prec**2
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_vae(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_vae, self).__init__()
    ...
    def reparameterize(self, mu, prec):
        """
    ...
    def forward(self, inputs):
        ...
        feat = self.reparameterize(feat, prec)
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_kld(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_prec_uncertainty_pooling_bottleneck_feature_kld, self).__init__()
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_logprec_uncertainty_pooling_bottleneck_feature_kld(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_logprec_uncertainty_pooling_bottleneck_feature_kld, self).__init__()
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        logprec = self.lin2(self.lin1_relu_bn(feat))   # frame log standard precision estimate (standard deviation)

        prec = torch.exp(logprec) # frame standard precision
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_logdev_uncertainty_pooling_bottleneck_feature_kld(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_logdev_uncertainty_pooling_bottleneck_feature_kld, self).__init__()
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        logdev = self.lin2(self.lin1_relu_bn(feat))   # frame log standard precision estimate (standard deviation)
        ...
        if self.stddev:
            sigma2 = torch.sum(torch.cat((feat, self.prior_mean.repeat(feat.shape[0], 1).unsqueeze(dim=2)), 2).pow(2) * weight_attn, dim=2)
            sigma = torch.sqrt(torch.clamp(sigma2 - phi ** 2, min=1.0e-10)) 
            return torch.cat((phi, sigma), dim=1).unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, dev**2
        else: 
            return phi.unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, dev**2
...
## Xi-vector pooling (softplus_prec) frame bottleneck feature
class xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature_kld(torch.nn.Module):

    def __init__(self, input_dim, output_dim, hidden_size=256, context=[0], stddev=False, train_mean=True, train_prec=True, dim_reduce_params={}):
        super(xivec_stdinit_softplus2_dev_uncertainty_pooling_bottleneck_feature_kld, self).__init__()
    ...
    def forward(self, inputs):
        ...
        if self.stddev:
            sigma2 = torch.sum(torch.cat((feat, self.prior_mean.repeat(feat.shape[0], 1).unsqueeze(dim=2)), 2).pow(2) * weight_attn, dim=2)
            sigma = torch.sqrt(torch.clamp(sigma2 - phi ** 2, min=1.0e-10)) 
            return torch.cat((phi, sigma), dim=1).unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, dev
        else: 
            return phi.unsqueeze(dim=2), Ls_inverse.unsqueeze(dim=2), feat, dev
