...
# Increase alignment.scp Zheng Li 2020-10
    ...
    def subset(self, num_utts:int, requirement:str="", drop=True, extra_list:list=[], seed:int=1024):
        ...
        if requirement == "--per-spk":
            ...
            if remain_num_utts > 0:
                utt_id_list.extend(list(np.random.choice(list(set(self.utt2spk.keys())-set(utt_id_list)), remain_num_utts, replace=False)))
            return self.filter(utt_id_list, id_type="utt")
...
class KaldiDatasetMultiTask():
    ...
    == Mapping files ==
        ...
        self.utt2dur: dict{str:float}
        self.utt2spk_int: dict{str:int}
        self.ali.scp: dict{str:str}        #Zheng Li 2020-10

    == Variables ==
        self.data_dir: str
        self.num_utts, self.num_spks, self.num_frames, self.feat_dim: int, self.ali_dim: int  #Zheng Li 2020-10
    """
    def __init__(self, data_dir:str="", 
                       expected_files:list=["utt2spk", "spk2utt", "feats.scp", "utt2num_frames", "ali.scp"], #Zheng Li 2020-10
                       more=False):
        # Fixed definition of str-first mapping files.
        # Tuple(Attr:str, FileName:str, Value_type:str, Vector:bool).
        self.utt_first_files = [
            ...
            ("vad_scp", "vad.scp", "str", False),
            ("text", "text", "str", True),
            ("ali_scp", "ali.scp", "str", False)]  #Zheng Li 2020-10
    ...
    def get_base_attribute_(self):
        ...
        # Total Alignment number (phone number) #Zheng Li 2020-10
        #self.phones = self.phones.items()
        #self.num_phones = int(self.phones[1]) if "phones" in self.loaded_attr else None
